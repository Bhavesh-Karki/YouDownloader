...
async function startDownload() {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/info?url=${encodeURIComponent(link)}`);
    ...
    status.innerText = "❌ Error connecting to the backend server. Make sure it is running on port 3000.";
    console.error(error);
  }
}
